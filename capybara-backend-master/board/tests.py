...
"""
The unit test cases for the board module.
"""
...
class BoardTests(TestCase):
    """
    The unit test case class for the board module.
    """
    # Initializer
    def setUp(self):
    ...
    # ! Utility functions
    def generate_jwt_token(self, payload: dict, salt: str) -> str:
        # * header
        header = {
            "alg": "HS256",
            "typ": "JWT"
    ...
    def generate_header(self, username: str, payload: dict = None, salt: str = SALT) -> dict:
        if payload is None:
            payload = {
                "iat": int(time.time()),
    ...
    def test_boards_post_new_twice_samename(self):
        ...
        self.assertTrue(User.objects.filter(name=user_name).exists())
        self.assertFalse(Board.objects.filter(
            board_name=board_name,
            board_state=board_state
        ).exists())
        self.assertTrue(Board.objects.filter(
    ...
    def test_user_board(self):
        res = self.client.get("/user/Ashitemaru")
        self.assertEqual(res.status_code, 200)
        self.assertGreater(len(res.json()['boards']), 0)
        self.assertFalse("board" in res.json()['boards'][0])

    def test_user_board_not_exist(self):
        res = self.client.get("/user/aaaa")
        self.assertEqual(res.status_code, 404)

    def test_user_board_method_not_allowed(self):
        res = self.client.post("/user/Ashitemaru")
        self.assertNotEqual(res.status_code, 200)
        res = self.client.put("/user/Ashitemaru")
        self.assertNotEqual(res.status_code, 200)
        res = self.client.delete("/user/Ashitemaru")
        self.assertNotEqual(res.status_code, 200)
